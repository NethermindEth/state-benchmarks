...
def start_infrastructure(client: str):
    ...
    run_cmd(["docker-compose", "-f", MONITORING_COMPOSE, "up", "-d"])
    
    logger.info(f"Starting {client} node...")
    run_cmd(["docker-compose", "-f", CLIENTS_COMPOSE, "up", "-d", client])
...
def stop_infrastructure():
    ...
    try:
        run_cmd(["docker-compose", "-f", CLIENTS_COMPOSE, "down", "-v"])
    except subprocess.CalledProcessError:
        pass
    try:
        run_cmd(["docker-compose", "-f", MONITORING_COMPOSE, "down", "-v"])
    except subprocess.CalledProcessError:
        pass
...
def run_locust_load_test(config_path: str, users: int = 20, spawn_rate: int = 10, run_time: str = "15s"):
    logger.info(f"Running Locust load test (Users: {users}, Run time: {run_time})...")
    # Updated to run via uv run locust
    subprocess.run([
        "uv", "run", "locust",
        ...
        "--run-time", run_time,
        "--host", RPC_URL,
        "--csv", "benchmarks/locust_stats"
    ], check=True, env=os.environ | {"LOCUST_CONFIG": config_path})
    logger.info("Locust load test completed.")

def measure_db_size(client: str) -> str:
...
def run_benchmark_for_client(client: str, milestone: str, skip_sync: bool, config_path: str):
    ...
    try:
        ...
        if wait_for_rpc():
            ...
            run_locust_load_test(config_path)
            
            logger.info("Invoking metrics aggregator...")
            # Updated to run via uv run python
            subprocess.run(["uv", "run", "python", "src/metrics/aggregator.py", "--config", config_path, "--client", client, "--milestone", milestone], check=True)
        else:
...
def main():
    ...
    parser.add_argument("--client", type=str, choices=["nethermind", "geth", "besu", "reth", "erigon"], help="Specific Ethereum client to benchmark (optional, defaults to config.yml list)")
